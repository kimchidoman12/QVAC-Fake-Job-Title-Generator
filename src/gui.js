#!/usr/bin/env node
// QVAC Fake Job Title Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29364;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/fakejobtitle") {
      try {
        ...
        const result = await generate(modelId, task.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake Job Title Generator GUI ready at http://localhost:${PORT}`);
  });
